...
def get_X_y_data():
    ...
    X = data_expression
    labels = pd.read_csv("TCGA_Barcode_breast.csv", header=0,
                         index_col=False)  # "TCGA_Barcode" Subtype_Selected"
...
def norm_data(X, method='zscore'):
    ...
    X_norm_df = pd.DataFrame(data=X_norm, index=list(X.index.values),
...
def clean_norm_data(X, PercentMean=25, PercentStd=20):
    ...
    reduceMeanNum = int(
        round((len(meanX) * PercentMean) / 100))  ### Calculates PercentMean% of X length

    stdX = np.std(X_arr, axis=0)
    sortColStd = stdX.argsort()  ### sort index from small to big by std
    reduceStdNum = int(
        round((len(stdX) * PercentStd) / 100))  ### Calculates PercentStd% of X length

    mask = np.isin(sortColStd[0:reduceStdNum], sortColMean[
                                               0:reduceMeanNum])  ### Finds elements that are both at PercentMean% low average and at PercentStd% low standard deviation
    idx = np.where(mask)
...
def Low_importance_Features(fs, cumulative_importance):
    ### non-deterministic
    fs.identify_zero_importance(task='classification', eval_metric='auc', n_iterations=3,
                                early_stopping=True)
...
def Select_Features(X, y, alpha=0.05, k=0, num_neighbors=0, threshold=0, isnormal=False,
                    isclean=False, corr_threshold=1, cumulative_importance=0):
...
if __name__ == "__main__":
    ...
    X_new = Select_Features(X, y, alpha=0.05, k=50, num_neighbors=3, threshold=0.3, isnormal=False,
                            isclean=False, corr_threshold=1, cumulative_importance=0)
