...
if __name__ == '__main__':
    ...
    dfs=[]
    model_name, y_pred, r2, MSE,score_train,score_test,EVS=linear(X_train,y_train,X_test,y_test)
    dfs.append(DataFrame({'model_name': model_name, 'r2': r2, 'MSE': MSE,'score_train':score_train,'score_test':score_test,'EVS':EVS}, index=[0]))
    model_name, y_pred, r2, MSE,score_train,score_test,EVS=Polynomial_linear(X_train,y_train,X_test,y_test,degree=3)
    dfs.append(DataFrame({'model_name': model_name, 'r2': r2, 'MSE':MSE,'score_train':score_train,'score_test':score_test,'EVS':EVS}))
    model_name, y_pred, r2, MSE,score_train,score_test,EVS=Decision_trees(X_train,y_train,X_test,y_test)
    dfs.append(DataFrame({'model_name': model_name, 'r2': r2, 'MSE': MSE,'score_train':score_train,'score_test':score_test,'EVS':EVS}, index=[0]))
    model_name, y_pred, r2, MSE,score_train,score_test,EVS=Random_forest_trees(X_train,y_train,X_test,y_test)
    dfs.append(DataFrame({'model_name': model_name, 'r2': r2, 'MSE': MSE,'score_train':score_train,'score_test':score_test,'EVS':EVS}, index=[0]))
    model_name, y_pred, r2, MSE,score_train,score_test,EVS=lasso_regression(X_train, y_train, X_test, y_test, lam=4)
    dfs.append(DataFrame({'model_name': model_name, 'r2': r2, 'MSE': MSE,'score_train':score_train,'score_test':score_test,'EVS':EVS}, index=[0]))
    dfs = pd.concat(dfs)
